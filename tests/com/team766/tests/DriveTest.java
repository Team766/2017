...
public class DriveTest extends RobotTestCase {
	...
	private void reset(){
		//Reset Encoders
		((tests.Encoder)instance.getEncoder(ConfigFile.getRightEncoder()[0], ConfigFile.getRightEncoder()[1])).set(0);
		((tests.Encoder)instance.getEncoder(ConfigFile.getLeftEncoder()[0], ConfigFile.getLeftEncoder()[1])).set(0);
		
		((tests.Gyro)instance.getGyro(ConfigFile.getGyro())).setAngle(0.0);
	}
}
